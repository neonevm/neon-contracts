...
library LibMetaplexProgram {
    ...
    // metadata account
    /// @param updateAuthority The public key of the account that currently has UPDATE authority over the metadata
    ...
    function formatUpdateMetadataAccountV2Instruction(
        ...
        // Include new update authority public key if needed
        if(newUpdateAuthority == bytes32(0)) {
            data = abi.encodePacked(
                data,
                bytes1(0x00) // Flag to indicate that no new update authority public key is provided
            );
        } else {
            data = abi.encodePacked(
                data,
                bytes1(0x01), // Flag to indicate that a new update authority public key is provided
                newUpdateAuthority
            );
        }
        // Add `primarySaleHappened` and `isMutable` fields
        data =  abi.encodePacked(
            data,
